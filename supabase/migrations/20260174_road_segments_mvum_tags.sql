-- Persist the MVUM-specific tag bag (per-vehicle-class flags, maintenance
-- level, etc.) so the road detail panel can surface the full MVUM data on
-- cache hits. Mirror to osm_tags for OSM tracks.
--
-- Also update get_road_segments to return seasonal_closure (which is saved
-- but was being dropped on read).

ALTER TABLE road_segments
  ADD COLUMN IF NOT EXISTS mvum_tags JSONB;
