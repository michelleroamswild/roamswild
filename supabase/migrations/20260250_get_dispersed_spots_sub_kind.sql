-- Surface kind + sub_kind from get_dispersed_spots so the explorer can
-- distinguish community-contributed spots (sub_kind='community') from
-- algorithmically-derived dead-ends (sub_kind='derived'). Both come back
-- as kind='dispersed_camping' today and the spot_type CASE collapses
-- everything that isn't sub_kind='known'/established_campground into
-- 'dead_end' — which means the map renders them with the same pin
-- color, despite being completely different provenance.
--
-- Drop-then-create because RETURNS TABLE shape is changing.
...
RETURNS TABLE (
    ...
    access_road JSONB,
    near_public_land_edge BOOLEAN,
    meters_from_public_land_edge NUMERIC,
    outside_public_land_polygon BOOLEAN,
...
BEGIN
    ...
    SELECT
        ...
        COALESCE((s.extra->>'near_public_land_edge')::BOOLEAN, FALSE) AS near_public_land_edge,
        (s.extra->>'meters_from_public_land_edge')::NUMERIC AS meters_from_public_land_edge,
        COALESCE((s.extra->>'outside_public_land_polygon')::BOOLEAN, FALSE) AS outside_public_land_polygon,
...
GRANT EXECUTE ON FUNCTION get_dispersed_spots(NUMERIC, NUMERIC, NUMERIC, TEXT, NUMERIC, BOOLEAN, INTEGER) TO anon, authenticated, service_role;
