-- Redirect the older derive_dead_end_spots fallback to write into `spots`.
-- This is only invoked when derive_spots_from_linked_roads errors, but we
-- want it consistent so any future fallback path doesn't repopulate the
-- legacy potential_spots table.
