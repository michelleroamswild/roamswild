-- Multiple historical signatures of insert_road_segment_simple exist in
-- the DB (each migration that added optional params created a new
-- overload while the old DROP only matched some signatures). When the
-- MVUM import path calls with the 7-param shape, PostgreSQL can't pick
-- between two matching candidates and errors.
--
-- Drop every known signature, then recreate the canonical 12-param version.
...
-- Recreate the canonical version (same as 20260198).
