-- Brute-force clean of all insert_road_segment_simple overloads, since
-- prior DROP FUNCTION attempts with explicit signatures didn't match every
-- historical variant. Drop every function with this name in `public`,
-- then recreate the single canonical 12-arg version.
...
BEGIN
    FOR fn_signature IN
        ...
        WHERE p.proname = 'insert_road_segment_simple'
          AND n.nspname = 'public'
    LOOP
        EXECUTE format('DROP FUNCTION public.insert_road_segment_simple(%s) CASCADE', fn_signature);
    END LOOP;
END $$;
